...
export default function DashDev() {
  ...
  const { data: tasks } = trpc.user.tasks.getByUser.useQuery(undefined, {
    ...
    <main className="h-screen w-screen">
      <section className="p-6">
        ...
        <div className="mb-4">
          ...
          {users ? (
            <div className="mt-2 grid min-h-20 grid-cols-2 gap-4 rounded-lg border border-emerald-500 p-2">
              {users?.map((user) => (
                <li
                  key={user.id}
                  className="flex items-center justify-between rounded-full border border-emerald-400/25 bg-white p-2"
                >
                  <button
                    key={user.id}
                    onClick={() => setSelectedUser(user.id)}
                    className="rounded-full border border-emerald-600 px-4 py-2 text-emerald-600 hover:bg-emerald-100 focus:outline-none"
                  >
                    {user.username}
                  </button>
                  <button
                    ...
                    className="border-error-400 text-error-400 hover:bg-error-400 rounded-full border px-4 py-2 hover:text-white focus:outline-none"
                  >
